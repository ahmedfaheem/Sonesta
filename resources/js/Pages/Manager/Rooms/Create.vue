...
<template>
    <Card class="max-w-3xl">
        <div class="border-b border-slate-200 px-6 py-5">
            <h2 class="text-xl font-semibold text-slate-950">Create room</h2>
            <p class="mt-1 text-sm text-slate-500">Room prices are entered in dollars here and stored in cents on save.</p>
        </div>
        ...
        <form class="space-y-6 px-6 py-6" @submit.prevent="submit">
            <div>
                ...
                <Input v-model="form.number" placeholder="1001" />
            ...
            <div class="grid gap-6 md:grid-cols-2">
                ...
                <div>
                    ...
                    <Input v-model="form.price" type="number" min="0.01" step="0.01" placeholder="50.00" />
            ...
            <div v-if="selectedFloor" class="rounded-2xl bg-slate-50 px-4 py-4 text-sm text-slate-600">
                This room will inherit ownership from floor #{{ selectedFloor.number }}.
            </div>

            <InputError :message="form.errors.room" />

            <div class="flex items-center gap-3">
                <Button type="submit" :disabled="form.processing || !floors.length">Create room</Button>
